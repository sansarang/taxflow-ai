...
const FAQ_ITEMS = [
  { q: "어떤 파일을 올리면 되나요?", a: "은행 앱에서 내려받은 거래내역 CSV 또는 영수증 사진을 올리시면 됩니다." },
    ...
    <div className="p-6">
      {error && (
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 mb-4 text-xs text-red-400">
          {error}
          {error.includes("회원가입") && <Link href="/signup" className="ml-2 underline font-bold">회원가입 →</Link>}
        </div>
      )}
      <div
      ...
      >
        <input ref={fileRef} type="file" accept=".csv,.xlsx,image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) startProcess(f) }} />
      ...
      <div className="mt-4 rounded-xl border border-white/[0.08] px-4 py-3 flex items-center justify-between" style={{ background: "#111216" }}>
        ...
        <span className="text-sm font-semibold">전체 공제 최적화</span>
      </div>
  ...
  if (phase === "uploading") return (
    <div className="p-8">
      ...
      <div className="relative h-12 rounded-xl overflow-hidden bg-white/10">
        <div className="absolute inset-y-0 left-0 transition-all duration-150 rounded-xl" style={{ width: uploadPct + "%", background: "#7c3aed" }} />
    ...
    <div className="p-8 text-center">
      ...
      <div className="relative w-32 h-32 mx-auto mb-8">
        <div className="absolute inset-0 flex items-center justify-center" style={{ animation: "spin 1.5s linear infinite" }}>
          <Image src="/logo.png" alt="미리택스" height={50} width={125} style={{ objectFit: "contain", opacity: 0.8 }} />
        </div>
        <div className="absolute inset-0 flex items-center justify-center">
    ...
    <div>
      <div className="border-b border-white/[0.06] py-4 px-5 sm:px-8" style={{ background: "#1a1c22" }}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <span className="text-sm font-bold">안녕하세요, {name}님 👋</span>
            <span className="ml-3 text-xs text-white/35">{profile.business_type || "크리에이터"}</span>
          </div>
          <Link href="/dashboard" className="text-xs text-violet-400 hover:text-violet-300 font-semibold">대시보드 →</Link>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div className="pt-2">
            <h1 className="text-4xl sm:text-5xl font-black leading-[1.05] tracking-tight mb-6">
              {name}님의 이번 달<br />세금을
              <span style={{ background: "linear-gradient(90deg,#a78bfa,#60a5fa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}> 미리 정리</span>해드립니다
            </h1>
            <p className="text-base text-white/45 leading-relaxed mb-6 max-w-md">거래내역을 올리면 AI가 즉시 분류하고 공제 항목을 찾아드립니다.</p>
          </div>
      ...
      <section className="border-t border-white/[0.06] py-12 px-5 sm:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid sm:grid-cols-4 gap-4">
            {[{ label: "거래내역", href: "/transactions", icon: "📊" }, { label: "AI 최적화", href: "/optimize", icon: "🤖" }, { label: "세금 리포트", href: "/export", icon: "📄" }, { label: "설정", href: "/settings", icon: "⚙️" }].map(m => (
              <Link key={m.href} href={m.href} className="rounded-2xl border border-white/[0.07] p-6 hover:border-white/20 transition-all" style={{ background: "#1a1c22" }}>
      ...
      {showModal && results && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(12px)" }} onClick={() => setShowModal(false)}>
          <div className="rounded-2xl border border-white/10 p-8 w-full max-w-md" style={{ background: "#1a1c22" }} onClick={e => e.stopPropagation()}>
...
function LandingView() {
  ...
  return (
    <div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <div className="pt-2">
            ...
            <p className="text-base sm:text-lg text-white/45 leading-relaxed mb-8 max-w-md">
              한국 크리에이터·1인사업자를 위한 실시간 AI 세금 코치. 놓친 공제를 찾고 매달 절세하세요.
            </p>
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {[
              ...
              <div key={p.name} className={`rounded-2xl p-7 border flex flex-col ${p.hi ? "border-violet-500/40" : "border-white/[0.07]"}`} style={{ background: p.hi ? "#1e1530" : "#1a1c22" }}>
                ...
                <Link href={p.href} className="block text-center py-3 rounded-xl font-bold text-sm transition-all" style={{ background: p.hi ? "#7c3aed" : "rgba(255,255,255,0.06)", color: p.hi ? "white" : "rgba(255,255,255,0.4)" }}>
                  {p.cta}
                </Link>
              </div>
            ))}
      ...
      <footer className="border-t border-white/[0.06] py-12 px-5 sm:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="grid sm:grid-cols-4 gap-8 mb-10">
            <div>
              <Image src="/logo.png" alt="미리택스" height={28} width={105} style={{ objectFit: "contain", opacity: 0.3, filter: "grayscale(1)" }} />
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6" style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(12px)" }} onClick={() => setShowModal(false)}>
          <div className="rounded-t-3xl sm:rounded-2xl border border-white/10 p-8 sm:p-10 w-full sm:max-w-md" style={{ background: "#1a1c22" }} onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-2xl font-black mb-3">결과를 저장하려면 회원가입이 필요합니다</h3>
            <p className="text-white/35 text-sm mb-8 leading-relaxed">
              내 실제 거래내역으로 무제한 분석하고 매달 절세 리포트를 받아보세요.<br />
              <span className="text-violet-400 font-semibold">첫 달 19,500원 · 언제든 해지 가능</span>
            </p>
            <div className="flex flex-col gap-3">
              <Link href="/signup?plan=pro" className="block text-center py-4 rounded-xl font-bold text-sm" style={{ background: "#7c3aed", color: "white" }}>첫 달 50% 할인으로 시작 →</Link>
              <Link href="/signup" className="block text-center py-3.5 rounded-xl font-semibold text-sm border border-white/10 text-white/40 hover:text-white transition-all">무료로 시작</Link>
              <button onClick={() => setShowModal(false)} className="text-xs text-white/20 hover:text-white/40 py-2">나중에</button>
            </div>
          </div>
        </div>
      )}
  ...
  useEffect(() => {
    async function checkSession() {
      try {
        ...
        const { data: p } = await (supabase as any).from("users_profile").select("id, full_name, business_type, is_simplified_vat, onboarding_completed").eq("id", user.id).single()
      ...
      <nav className="border-b border-white/[0.08] sticky top-0 z-50" style={{ background: "rgba(17,18,22,0.95)", backdropFilter: "blur(20px)" }}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Image src="/logo.png" alt="미리택스" height={36} width={135} style={{ objectFit: "contain" }} priority />
