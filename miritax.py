...
if logo_files:
    ...
    dest = f'public/logo.{ext}'
    shutil.copy2(src, dest)
    logo_path = f'/logo.{ext}'
    print(f'Logo copied: {src} -> {dest}')
else:
    logo_path = None
...
const PAIN_POINTS = [
  {{
    no: "01",
    q: "매달 거래내역 정리하는 게 너무 귀찮고 시간 많이 드시나요?",
    solution: "CSV 한 번 올리면 AI가 자동 분류",
    demo: "auto_classify",
  }},
  {{
    no: "02",
    q: "뭐가 공제되는지 몰라서 나중에 후회하시나요?",
    solution: "놓친 공제 항목 즉시 감지 & 알림",
    demo: "deduction",
  }},
  {{
    no: "03",
    q: "신고 기간에 급하게 하다 실수할까봐 불안하시나요?",
    solution: "분기별 예정신고 자동 알림",
    demo: "alert",
  }},
  {{
    no: "04",
    q: "증빙 자료 찾는 게 번거로우시나요?",
    solution: "사진 OCR로 영수증 즉시 등록",
    demo: "ocr",
  }},
  {{
    no: "05",
    q: "이번 달 세금 얼마나 나올지 모르시죠?",
    solution: "실시간 세금 예측 대시보드",
    demo: "forecast",
  }},
]
...
function Logo({{ size = "md", gray = false }}: {{ size?: "sm"|"md"|"lg", gray?: boolean }}) {{
  const heights: Record<string, string> = {{ sm: "h-7", md: "h-9", lg: "h-12" }}
  {"if logo_path:" if logo_path else "# no logo"}
  return (
    <Image
      src="{logo_path or '/logo.png'}"
      alt="미리택스"
      height={{size === "lg" ? 48 : size === "sm" ? 28 : 36}}
      width={{size === "lg" ? 180 : size === "sm" ? 105 : 135}}
      className={{`${{heights[size]}} w-auto ${{gray ? "opacity-40 grayscale" : ""}}`}}
      ...
      <div className="space-y-2">
        {{items.map((item, i) => (
          <div key={{item}} className={{`flex items-center gap-2 text-xs px-3 py-2 rounded-lg transition-all duration-500 ${{tick % 4 >= i ? "bg-violet-500/20 text-violet-300" : "bg-white/[0.04] text-white/25"}}`}}>
            <span className={{`w-1.5 h-1.5 rounded-full flex-shrink-0 ${{tick % 4 >= i ? "bg-violet-400" : "bg-white/20"}}`}} />
            {{item}}
          </div>
        ))}}
      </div>
    ...
    return (
      <div className="space-y-3">
        {{[
          ...
          {{ label: "종합소득세 신고", date: "5월 31일", done: false, urgent: false }},
          {{ label: "2분기 예정신고", date: "7월 25일", done: false, urgent: false }},
        ].map(a => (
  ...
  if (demo === "ocr") {{
    ...
    return (
      <div className="text-center space-y-3">
        <div className="w-16 h-20 rounded-lg border border-white/10 bg-white/[0.03] mx-auto flex items-center justify-center text-3xl">
          🧾
        </div>
        <div className="space-y-1.5">
          {{["사진 인식 중...", "텍스트 추출 중...", "거래내역 등록 완료!", "공제 항목 확인됨!"].map((s, i) => (
    ...
    return (
      <div className="space-y-3">
        <div className="flex justify-between text-xs text-white/40 mb-1">
          <span>이번 달 예상 세금</span>
          <span className="text-violet-400 font-bold animate-pulse">계산 중...</span>
        </div>
        <div className="text-2xl font-black text-white">
          {{(progress * 3200).toLocaleString()}}원
        </div>
        <div className="h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
          <div className="h-full rounded-full bg-violet-500 transition-all duration-700" style={{{{ width: `${{progress}}%` }}}} />
        </div>
        <div className="grid grid-cols-3 gap-2 text-center">
          {{[["소득세", "18만"], ["4대보험", "9만"], ["부가세", "5만"]].map(([l, v]) => (
            <div key={{l}} className="rounded-lg bg-white/[0.04] p-2">
              <div className="text-[10px] text-white/30">{{l}}</div>
              <div className="text-xs font-bold text-white/70">{{v}}</div>
            </div>
          ))}}
        </div>
      </div>
          ...
          <div className="rounded-2xl border border-white/[0.1] overflow-hidden" style={{{{ background:"#1a1c22" }}}}>
            <div className="px-6 py-5 border-b border-white/[0.07] flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-white/30 mt-0.5">CSV 또는 영수증 사진을 올리세요</p>
              </div>
            ...
            {{step === "idle" && (
              <div className="p-6">
                ...
                >
                  ...
                  <p className="text-xs text-white/25">거래내역 CSV · 영수증 JPG/PNG</p>
                </div>
                ...
                <div className="mt-4 rounded-xl border border-white/[0.08] px-4 py-3 flex items-center justify-between" style={{{{ background:"#111216" }}}}>
                  ...
                  <span className="text-sm font-semibold flex items-center gap-2">전체 공제 최적화
                ...
                >파일 선택하기</button>
                <button onClick={{runSample}} className="mt-3 w-full text-xs text-white/25 hover:text-white/50 transition-colors py-2">파일 없이 샘플로 체험하기</button>
                    ...
                    {{results.map((r,i) => (
                      <div key={{i}} className="px-4 py-3 flex items-center justify-between">
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          <span className={{`text-[10px] font-bold ${{r.deductible ? "text-violet-400" : "text-white/20"}}`}}>{{r.deductible ? "공제가능" : "공제불가"}}</span>
                        </div>
                      </div>
                    ))}}
                  </div>
                </div>
                <button onClick={{() => setShowModal(true)}} className="w-full py-3.5 rounded-xl font-bold text-sm mb-2 transition-all active:scale-95" style={{{{ background:"#7c3aed" }}}} onMouseEnter={{e=>(e.currentTarget.style.background="#6d28d9")}} onMouseLeave={{e=>(e.currentTarget.style.background="#7c3aed")}}>결과 저장 및 PDF 다운로드</button>
                <button onClick={{()=>{{setStep("idle");setResults([]);setFileName(null)}}}} className="w-full text-xs text-white/25 hover:text-white/50 py-2 transition-colors">다시 분석하기</button>
              </div>
            )}}
          </div>
        </div>
      ...
      <section id="pain" className="border-t border-white/[0.06] py-24 px-5 sm:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-white/35 text-sm sm:text-base">직접 움직이며 해결하는 모습을 확인해보세요</p>
          </div>
          <div className="space-y-6">
            {{PAIN_POINTS.map((p, idx) => (
              <div key={{p.no}} className="grid md:grid-cols-2 gap-6 rounded-2xl border border-white/[0.07] overflow-hidden" style={{{{ background:"#1a1c22" }}}}>
      ...
      <section className="py-24 px-5 sm:px-8 border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="rounded-2xl border border-white/[0.08] p-6" style={{{{ background:"#1a1c22" }}}}>
            ...
            <button onClick={{()=>{{window.scrollTo({{top:0,behavior:"smooth"}});setStep("idle")}}}}
              className="w-full py-4 rounded-xl font-bold text-base transition-all active:scale-95"
              style={{{{ background:"#7c3aed" }}}}
              onMouseEnter={{e=>(e.currentTarget.style.background="#6d28d9")}}
              onMouseLeave={{e=>(e.currentTarget.style.background="#7c3aed")}}>파일 선택하기</button>
      ...
      <footer className="border-t border-white/[0.06] py-12 px-5 sm:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="grid sm:grid-cols-4 gap-8 mb-10">
            ...
            <div>
              <p className="text-xs font-semibold text-white/40 mb-3 uppercase tracking-wider">서비스</p>
              <ul className="space-y-2 text-xs text-white/25">
                <li><Link href="/dashboard" className="hover:text-white/50">대시보드</Link></li>
                <li><Link href="/upload" className="hover:text-white/50">거래내역 업로드</Link></li>
                <li><Link href="/optimize" className="hover:text-white/50">AI 최적화</Link></li>
              </ul>
            </div>
            <div>
              <p className="text-xs font-semibold text-white/40 mb-3 uppercase tracking-wider">회사</p>
              <ul className="space-y-2 text-xs text-white/25">
                <li><a href="#" className="hover:text-white/50">소개</a></li>
                <li><a href="#faq" className="hover:text-white/50">FAQ</a></li>
                <li><a href="#" className="hover:text-white/50">이용약관</a></li>
              </ul>
            </div>
            <div>
              <p className="text-xs font-semibold text-white/40 mb-3 uppercase tracking-wider">요금제</p>
              <ul className="space-y-2 text-xs text-white/25">
                <li><Link href="/signup" className="hover:text-white/50">Starter</Link></li>
                <li><Link href="/signup?plan=pro" className="hover:text-white/50">Pro</Link></li>
                <li><Link href="/signup?plan=business" className="hover:text-white/50">Business</Link></li>
              </ul>
            </div>
          </div>
